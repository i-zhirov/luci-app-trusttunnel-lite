...
- Re-review (attempt 2): all 5 prior findings verified Resolved (setup_trust_store in Task 4, real oracle counts 29/21, 26 keys pinned, routing_profile.* branch in the body, profile-edit checklist item).
- New on re-review (informational, both): the Summary's "486 lines" is stale (the file is 493 lines at c43e20a); the Actualization's NOTE that issue.md "still says 25-key" is stale (issue.md line 98 already reads "26-key schema threshold"). Neither affects the implementation path.
