...
- Re-review (attempt 2): all 8 prior findings verified Resolved (status key set, profile-aware check_domain, diagnose counts 17/18, shape-parser import, profile golden scenarios, R1/D2/D9, reusable docker gate image, rootfs stub injection).
- Two wording defects found on re-review were FIXED in the plan and verified by the primary agent immediately after the review: (1) the vercmp probe matrix now expects `abc` vs `1.0.0` → −1 (the unparseable side becomes [0] via the numeric-match fallback and loses to [1,0,0] — verified against the current vercmp); (2) the two stale `.gitignore` change expectations were removed (the resolved note for finding 7 said the change was dropped).
