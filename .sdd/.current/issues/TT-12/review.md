...
- Re-review (attempt 2): both prior findings verified Resolved (profile checklist items 8-9 with the raw pass-through strings; both unmapped backend strings named in the actualization).
- New on re-review (informational): checklist items 8 and 9 assert the count "is at the 18-entry max" without stating the precondition — the 18th entry is the conditional MTU-mismatch warn; a healthy device yields 17. Rephrase to "17 entries (18 only when the MTU mismatch check fires)" or "up to the 18-entry max".
