...
- Re-review (attempt 2): all 5 prior findings verified Resolved (Task 8 around the real assemble steps, D10/site-layout contract, 7-entry template set, re-cited baseline, per-arch live checklist).
- New on re-review (cosmetic, FIXED in the plan by the primary agent): the Actualization's "verification comments changed (kmod-tun/ca-bundle/nftables wording)" did not match the workflow (the comments say "luci-base, kmod-tun, ca-bundle, curl, ..." — no nftables); the plan text now quotes the actual wording.
