...
- Re-review (attempt 2): all 7 prior findings verified Resolved (actualized tasks, rewritten probe green on the rebased file, 79 verified keys — the prior review's "77" was itself wrong, spec-of-record actualized, manual checklist, unguarded access pinned).
- New on re-review (informational): the probe pins token presence for the NamedSection declarations but not the 4-arg argument structure (`form.NamedSection, 'main', 'main', _('General')`) — a 3-arg form would pass the probe but bind the sectiontype to the title; have the scaffold group assert the sectiontype argument equals the section id.
